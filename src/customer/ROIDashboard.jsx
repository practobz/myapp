...
const ROIDashboard = () => {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      // Determine time period in days based on selected timeframe
      const timePeriodDays = timeframe === 'last_7_days' ? 7 : timeframe === 'last_90_days' ? 90 : 30;

      // ── PRIMARY: Unified Analytics Collector data ─────────────────────────
      try {
        console.log('📊 Fetching unified analytics data from collector...');
        const unifiedResponse = await fetch(
          `${backendUrl}/api/analytics/data?customerId=${customerId}&days=${timePeriodDays}`,
          {
            method: 'GET',
        ...
        console.log('📊 Unified analytics response status:', unifiedResponse.status);

        if (unifiedResponse.ok) {
          const unifiedResult = await unifiedResponse.json();
          console.log('📊 Unified analytics records:', unifiedResult.count);

          if (unifiedResult.success && unifiedResult.data && unifiedResult.data.length > 0) {
            const processedData = processUnifiedAnalyticsData(unifiedResult.data);

            if (processedData && Object.keys(processedData).length > 0) {
              const connectedPlatforms = Object.keys(processedData);

              // Build connected-accounts list from the raw docs for account selector
              const accountsFromDocs = [];
              const seenIds = new Set();
              unifiedResult.data.forEach(doc => {
                const uid = `${doc.platform}_${doc.accountId}`;
                if (!seenIds.has(uid)) {
                  seenIds.add(uid);
                  accountsFromDocs.push({
                    id: uid,
                    platform: doc.platform,
                    accountId: doc.accountId,
                    name: doc.accountName || `${doc.platform} account`,
                    profilePicture: null,
                    type: 'collected'
                  });
                }
              ...
              setConnectedAccounts(accountsFromDocs);
              setDashboardData(processedData);
              setAnalyticsData({
                customerId,
                platforms: connectedPlatforms,
                isUnified: true,
                recordCount: unifiedResult.count
              });
              setPlatforms(connectedPlatforms);
              setLastUpdated(new Date().toISOString());
              setIsLoading(false);
              console.log('✅ Unified analytics data loaded successfully for platforms:', connectedPlatforms);
              return;
            }
          } else {
            console.log('📊 No unified analytics records found for customer, falling back...');
          }
        }
      } catch (unifiedError) {
        console.log('📊 Unified analytics fetch failed:', unifiedError.message);
      }

      // ── FALLBACK 1: historical_data API ──────────────────────────────────
  ...
  // Process analytics_data documents collected by unifiedAnalyticsCollector into dashboard format
  const processUnifiedAnalyticsData = (docs) => {
    console.log('🔄 Processing unified analytics data:', docs.length, 'documents');

    if (!docs || docs.length === 0) return null;

    // Group by platform, then sort ascending by collectedAt
    const byPlatform = {};
    docs.forEach(doc => {
      const p = doc.platform;
      if (!p) return;
      if (!byPlatform[p]) byPlatform[p] = [];
      byPlatform[p].push(doc);
    });

    const result = {};

    Object.keys(byPlatform).forEach(platform => {
      const sorted = [...byPlatform[platform]].sort(
        (a, b) => new Date(a.collectedAt) - new Date(b.collectedAt)
      );

      // Most recent snapshot = "current", oldest = "previous"
      const latest = sorted[sorted.length - 1];
      const oldest = sorted[0];
      const m = latest.metrics || {};
      const mPrev = oldest.metrics || {};
      ...
      const metric = (cur, prev) => ({
        current: cur || 0,
        previous: prev || 0,
        growth: calcGrowth(cur || 0, prev || 0)
      });

      // Build timeline arrays for charts (follower/subscriber counts over time)
      const followerKey = platform === 'youtube' ? 'subscribers' : 'followers';
      const monthlyData = sorted.map(d => d.metrics?.[followerKey] || d.metrics?.followers || 0);
      const growthTrend = sorted.map(d => ({
        date: d.collectedAt,
        value: d.metrics?.[followerKey] || d.metrics?.followers || 0
      }));

      const platformData = { monthlyData, growthTrend };

      if (platform === 'facebook') {
        platformData.followers        = metric(m.followers, mPrev.followers);
        platformData.likes            = metric(m.totalLikes, mPrev.totalLikes);
        platformData.comments         = metric(m.totalComments, mPrev.totalComments);
        platformData.shares           = metric(m.totalShares, mPrev.totalShares);
        platformData.postCount        = m.postCount || 0;
        platformData.reach            = metric(m.pageImpressions, mPrev.pageImpressions);
        platformData.impressions      = metric(m.pageImpressions, mPrev.pageImpressions);

      } else if (platform === 'instagram') {
        platformData.followers        = metric(m.followers, mPrev.followers);
        platformData.likes            = metric(m.totalLikes, mPrev.totalLikes);
        platformData.comments         = metric(m.totalComments, mPrev.totalComments);
        platformData.mediaCount       = m.mediaCount || 0;
        platformData.reach            = metric(m.reach, mPrev.reach);
        platformData.impressions      = metric(m.impressions, mPrev.impressions);
        platformData.profileViews     = metric(m.profileViews, mPrev.profileViews);

      } else if (platform === 'youtube') {
        platformData.subscribers      = metric(m.subscribers, mPrev.subscribers);
        platformData.views            = metric(m.recentTotalViews || m.totalViews, mPrev.recentTotalViews || mPrev.totalViews);
        platformData.likes            = metric(m.recentTotalLikes, mPrev.recentTotalLikes);
        platformData.comments         = metric(m.recentTotalComments, mPrev.recentTotalComments);
        platformData.videoCount       = m.videoCount || 0;
        platformData.totalViews       = metric(m.totalViews, mPrev.totalViews);

      } else if (platform === 'linkedin') {
        platformData.followers        = metric(m.followers, mPrev.followers);
        platformData.connections      = metric(m.connections, mPrev.connections);
        platformData.impressions      = metric(m.impressions || m.uniqueImpressions, mPrev.impressions || mPrev.uniqueImpressions);
        platformData.likes            = metric(m.totalLikes, mPrev.totalLikes);
        platformData.comments         = metric(m.totalComments, mPrev.totalComments);
        platformData.shares           = metric(m.totalShares, mPrev.totalShares);
        platformData.postCount        = m.postCount || 0;
      ...
      // Calculate engagement rate for all platforms
      const followerCount = (platformData.subscribers || platformData.followers)?.current || 0;
      const totalEng = (platformData.likes?.current || 0) +
                       (platformData.comments?.current || 0) +
                       (platformData.shares?.current || 0);
      const prevTotalEng = (platformData.likes?.previous || 0) +
                           (platformData.comments?.previous || 0) +
                           (platformData.shares?.previous || 0);
      const prevFollowers = (platformData.subscribers || platformData.followers)?.previous || 0;

      if (followerCount > 0) {
        const engRate = (totalEng / followerCount) * 100;
        const prevEngRate = prevFollowers > 0 ? (prevTotalEng / prevFollowers) * 100 : engRate;
        platformData.engagement_rate = {
          current: Math.round(engRate * 100) / 100,
          previous: Math.round(prevEngRate * 100) / 100,
          growth: calcGrowth(engRate, prevEngRate)
        };
      }
    ...
    console.log('✅ processUnifiedAnalyticsData result:', Object.keys(result));
